...
(function($) {
    ...
    window.SelectFilter = {
        init: function(field_id, field_name, is_stacked) {
            ...
            var li_chooseall_link = quickElement('li', selector_chooser);
            li_chooseall_link.className = 'selector-chooseall-middle';
            var choose_all = quickElement('a', li_chooseall_link,
                gettext('Choose all'), 'title',
                interpolate(gettext('Click to choose all %s at once.'), [field_name]),
                'href', '#', 'id', field_id + '_add_all_link');
            choose_all.className = 'selector-chooseall';

            var li_add_link = quickElement('li', selector_chooser);
            li_add_link.className = 'selector-middle';
            var add_link = quickElement('a', li_add_link, gettext('Choose'), 'title', gettext('Choose'), 'href', '#', 'id', field_id + '_add_link');
            add_link.className = 'selector-add';

            var li_remove_link = quickElement('li', selector_chooser);
            li_remove_link.className = 'selector-middle';
            var remove_link = quickElement('a', li_remove_link, gettext('Remove'), 'title', gettext('Remove'), 'href', '#', 'id', field_id + '_remove_link');
            remove_link.className = 'selector-remove';

            var li_clearall_link = quickElement('li', selector_chooser);
            li_clearall_link.className = 'selector-clearall-middle';
            var clear_all = quickElement('a', li_clearall_link, gettext('Remove all'), 'title', interpolate(gettext('Click to remove all chosen %s at once.'), [field_name]), 'href', '#', 'id', field_id + '_remove_all_link');
            clear_all.className = 'selector-clearall';

            // css for chooser
            var dis = (0.96 * selector_chooser.getBoundingClientRect().width - remove_link.getBoundingClientRect().width * 2 - choose_all.getBoundingClientRect().width - clear_all.getBoundingClientRect().width) / 3;
            choose_all.style.marginRight = dis.toString()+"px";
            choose_all.style.paddingLeft = (dis/2).toString()+"px";
            clear_all.style.marginLeft = dis.toString()+"px";
            ...
            var add_new = quickElement('div', selector_div);
            add_new.className = 'add-new';
            var add_new_link = quickElement('a', add_new, gettext('Create new format '), 'href', '/payment-formats/add-payment-format');
            add_new_link.className = 'add-new-link';
            quickElement(
                'span', add_new_link, '',
                'class', 'add-new-link-symbol'
            );
